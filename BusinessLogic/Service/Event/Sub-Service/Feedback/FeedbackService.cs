...
namespace BusinessLogic.Service.Event.Sub_Service.Feedback
{
	public class FeedbackService : IFeedBackService
	{
		...
		public FeedbackService(IUnitOfWork unitOfWork, DeepLearningService.DLService dlService, IEventService eventService)
		{
		...
		public async Task<EventFeedbackSummaryDto> GetEventFeedbackSummary(string eventId)
		{
			...
			EventId = eventEntity.Id,
				...
				Rating = latest?.RatingEvent,  // enum — rating của feedback mới nhất
				AvgRating = feedbacks.Count == 0 ? 0 : Math.Round(feedbacks.Average(x => (double)(int)x.RatingEvent), 2),
		...
		public Task<List<EventFeedbackSummaryDto>> ShowFeedbackForOrganizer(string userId, FeedbackForOrganizerDTO dto) => _eventService.ShowFeedbackForOrganizer(userId, dto);
